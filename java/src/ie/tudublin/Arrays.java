...
public class Arrays extends PApplet {
    ...
    public float map1(float from, float start1, float stop1, float start2, float stop2)
    {
        ...
        float per;
        float result;
        per = from / (start1 + stop1);
        result = (start2 + stop2) * per;
        return result;
    }
    ...
    public void setup() {
        ...
        // What month had most rainfall and what month has the least 
        int minIndex = 0;
        int maxIndex = 0;
        float sum = 0;
        for(int i = 0; i <rainfall.length; i++)
        ...
        // bar chart for the rainfall using the map function 
        colorMode(HSB);
        background(0);
